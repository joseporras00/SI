...
function validar(f) {
  ...
  msg += validarDestinatario(f.Destinatario.value);
  msg += validarCuerpo(f.Cuerpo.value);
  function validarTitulo(inp){
	  if (inp == "")
		  return “No se introdujo titulo del anuncio\n";
		  else if (inp.length < 5)
		  return “titulo demasiado corto\n";
		  else return "";
  }
  function validarDestinatario(inp){
	  if (inp == "") return “No se introdujo email del destinatario\n";
  ...
  function validarCuerpo(inp){
	  if (inp == "") return “No se introdujo cuerpo del anuncio\n";
	  else if (inp.length < 5)
	  return “Cuerpo de anuncio demasiado corto\n";
	  else return ""; 
  }
  alert(msg);
  return ok;
...
/* Posibles flujos:
	...
	*/
String nextPage = "/Practica3/crearAnuncio";
...
if (customerBean == null || customerBean.getEmailUser().equals("")) {
	...
	session.setAttribute("Tipo", "Individualizado");
%>
<div class="msg"><%= messageNextPage %></div>
<br/><br/>
<form novalidate method="post" action="/Practica3/crearAnuncio" onsubmit="return validar(this);">
	<h2>Crear Anuncio</h2>
